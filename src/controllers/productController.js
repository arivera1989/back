...
export const getProductsPlain = async (req) => {
    await ProductManager.loadProducts()
    const limit = parseInt(req.query.limit)
    const products = (!isNaN(limit) && limit > 0) ? ProductManager.products.slice(0, limit) : ProductManager.products
    return products
...
export const deleteProduct = async (req, res) => {
    const productId = parseInt(req.params.pid);
    const productExists = ProductManager.findProduct(productId);
    ...
    await ProductManager.saveProducts();
    return res.status(200).json({ success: `Producto con id ${productId} eliminado.` });
};
